pipeline {
  ...
  stages {
    ...
    stage('Generate docker image for development') {
      ...
      steps {
        sh 'docker build -t $DOCKER_REGISTRY/entropypool/ohash-dashboard-v2:latest .'
      }
    }
    ...
    stage('Generate docker image for testing or production') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          PATH=/usr/local/bin:$PATH:./node_modules/@quasar/app/bin yarn install --registry https://registry.npm.taobao.org/
          PATH=/usr/local/bin:$PATH:./node_modules/@quasar/app/bin quasar build
          docker build -t $DOCKER_REGISTRY/entropypool/ohash-dashboard-v2:$tag .
        '''.stripIndent())
      }
    }
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh 'docker push $DOCKER_REGISTRY/entropypool/ohash-dashboard-v2:latest'
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep ohash-dashboard-v2 | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep ohash-dashboard-v2 | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            docker push $DOCKER_REGISTRY/entropypool/ohash-dashboard-v2:$tag
          fi
        '''.stripIndent())
      }
    }
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/ohash-dashboard-v2:latest/ohash-dashboard-v2:$tag/g" k8s/01-ohash-dashboard.yaml
